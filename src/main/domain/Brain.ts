...
export class Brain {
  ...
  public analyse(
    ...
    path: string,
    title: string,
    poster: Attachment | undefined,
    containerTitle: string | undefined,
    containerAttachments: Attachment[] | undefined,
    tagCount: number,
    ...
    for (const track of tracks) {
      switch (track.type) {
        ...
        case TrackType.AUDIO: {
          const { changes: c, hints: h } = this.analyseAudioTrack(
            track,
        ...
        case TrackType.SUBTITLES: {
          const { changes: c, hints: h } = this.analyseSubtitlesTrack(
            track,
            originalLanguageIETF,
            originalCountries,
            userHints
    ...
    const { changes: c } = this.analyseContainer(
      path,
      title,
      tmdb,
      edition,
  ...
  private guessTrackLanguage(
    track: Track,
    originalLanguageIETF: LanguageIETF | undefined,
    originalCountries: Country[],
    nbAudioTracks: number,
    audioVersions: AudioVersion[],
    userHints: Hint[]
  ) {
    ...
    const changes: Change[] = []
    const hints: Hint[] = []
    let languageHint: string | undefined = undefined
      ...
      if (
        ...
        if (languageInfo !== undefined) {
          if (languageInfo.isRegionImportant) {
            ...
            languageHint = trackLanguage
              ? Languages.guessLanguageIETFFromCountries(trackLanguage, originalCountries)?.code
              : trackLanguage
          }
        } else {
          languageHint = ''
    ...
    if (
      ...
      if (userLanguageHint && track.language !== userLanguageHint) {
        changes.push(
          new Change(
            track.type as unknown as ChangeSourceType,
            ChangeType.UPDATE,
            track.id,
            ChangeProperty.LANGUAGE,
            track.language,
  ...
  private analyseSubtitlesTrack(
    ...
    const { changes: c, hints: h } = this.guessTrackLanguage(
      track,
      originalLanguageIETF,
      originalCountries,
      -1,
