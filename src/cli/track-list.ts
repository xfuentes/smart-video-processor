...
import {booleanRenderer, codecRenderer, durationRenderer, framesRenderer, sizeRenderer, trackPropertiesRenderer, trackTypeRenderer} from "./renderers.ts";
...
export const requestTracksToCopy = async (outputBuffer: CLI.LineBuffer, video: Video, auto: boolean = false) => {
    if (!auto) {
        ...
        const choices: { name: string, value: string | number | undefined, checked: boolean, disabled?: string | boolean, short?: string }[] = [];
