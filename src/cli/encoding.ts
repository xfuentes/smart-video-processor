...
import {TrackType} from "../common/Track.ts";
    ...
    if (filteredTracks.length > 0) {
        for(const track of filteredTracks) {
            ...
            if(disabled) {
                ...
                if(auto) {
                    console.log(`${chalk.greenBright("✔  ")}${chalk.whiteBright(`Encode ${key}?`)} ${chalk.blueBright(video.isTrackEncodingEnabled(key) ? "Yes" : "No")}`);
                } else {
                    video.setTrackEncodingEnabled(key, await confirm({
                        message: ` Encode ${key}?`,
