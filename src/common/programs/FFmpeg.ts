...
import {EncoderSettings, VideoCodec} from "../Encoding.ts";
...
export class FFmpeg extends CommandProgress {
    ...
    public async encodeFile(path: string, durationSeconds: number, tracks: Track[], settings: EncoderSettings[], progressNotifier?: ProgressNotifier): Promise<string> {
        const uuid = UUIDv4();
            ...
            if (progressNotifier !== undefined) {
                ...
                if (progress === undefined) {
                    ...
                    if (currentPass === 1) {
                        ...
                        progressNotifier({progress: totalProgress, xSpeed, countdown, pass: currentPass, process});
                    } else {
        ...
        await this.encodeFileInternal(uuid, path, durationSeconds, tracks, settings, currentPass, progressNotifierAggregator);
        ...
        const prom = this.encodeFileInternal(uuid, path, durationSeconds, tracks, settings, currentPass, progressNotifierAggregator);
        await prom.finally(() => {
    ...
    async encodeFileInternal(uuid: string, path: string, durationSeconds: number, tracks: Track[], settings: EncoderSettings[], pass?: number, progressNotifier?: ProgressNotifier): Promise<string> {
        durationSeconds = currentSettings.isTestEncodingEnabled ? 30 : durationSeconds;
        if(progressNotifier) {
            progressNotifier({progress: undefined, pass});
        }
        const encodedPath = pathJoin(JobManager.getInstance().getTempPath("encoded"), uuid + ".mkv");
        const args = this.generateEncodingArguments(path, encodedPath, tracks, settings, pass);
        ...
        try {
            ...
            if ((error as Error).message.indexOf("Too many packets buffered") != -1) {
                const workaroundArgs = this.generateEncodingArguments(path, encodedPath, tracks, settings, pass, true);
    ...
    private generateEncodingArguments(path: string, encodedPath: string, tracks: Track[], settings: EncoderSettings[], pass: number | undefined = undefined,
                                      maxMuxingQueueSizeWorkaround: boolean = false): string[] {
