...
import Processes, {ProgressNotifier} from "../../util/processes.ts";
import Track, {TrackType} from "../Track.ts";
import Numbers from "../../util/numbers.ts";
import Languages from "../LanguageIETF.ts";
import {Attachment, Change, ChangeProperty, ChangeSourceType, ChangeType} from "../Change.ts";
import Files from "../../util/files.ts";
import {pathIsAbsolute, pathJoin} from "../../util/path.ts";
import {CommandProgress} from "./CommandProgress.ts";
...
export class MKVMerge extends CommandProgress {
    ...
    public async retrieveFileInformation(path: string): Promise<{ tracks: Track[], container: Container }> {
        ...
        mkvInfo?.tracks?.forEach((track) => {
            ...
            if (type !== undefined) {
                const language = track.properties.language_ietf ??
                    (track.properties.language === "und" || track.properties.language === undefined ? undefined : (Languages.getLanguageByCode(track.properties.language)?.code));
                ...
                tracks.push(new Track(track.id, track.properties.track_name, type, track.codec, language, {
                    ...
                    }, track.properties.default_track, track.properties.forced_track,
        ...
        const container: Container = {
            ...
            attachments: !mkvInfo?.attachments ? [] : mkvInfo?.attachments?.map(mkva => new Attachment(mkva.file_name, mkva.content_type, mkva.description)),
    ...
    async processFile(originalFilename: string, path: string, outputDirectory: string, changes: Change[], tracks: Track[] = [], progressNotifier?: ProgressNotifier): Promise<string> {
        ...
        const outputInterpreter = (stdout?: string, stderr?: string, process?: ChildProcess): { response: string, error?: string } => {
            let response = "";
    ...
    private generateProcessingArguments(originalFilename: string, path: string, outputDirectory: string, changes: Change[], tracks: Track[]): string[] {
        ...
        let newFilename = changes.find(c =>
            c.sourceType === ChangeSourceType.CONTAINER &&
            c.changeType === ChangeType.UPDATE &&
            c.property === ChangeProperty.FILENAME)?.newValue as string | undefined ?? "";

        if (!newFilename) {
            newFilename = originalFilename;
        }
        newFilename = Files.removeSpecialCharsFromFilename(newFilename);
        newFilename = pathJoin(outputDirectory, newFilename);
        ...
        return mkOptions.concat(["(", path, ")"]);
    }

    private generateGlobalArguments(changes: Change[]): string[] {
        const mkOptions: string[] = [];

        const newContainerTitle = changes.find(c =>
            c.sourceType === ChangeSourceType.CONTAINER &&
            c.changeType === ChangeType.UPDATE &&
            c.property === ChangeProperty.TITLE)?.newValue as string | undefined;
    ...
    private generateAttachmentArguments(changes: Change[]): string[] {
        ...
        const clearAttachmentsRequest = changes.find(c =>
            c.sourceType === ChangeSourceType.CONTAINER &&
        ...
        const updatePosterRequest = changes.find(c =>
            c.sourceType === ChangeSourceType.CONTAINER &&
            c.changeType === ChangeType.UPDATE &&
    ...
    private generateTracksArguments(tracks: Track[], changes: Change[]): string[] {
        ...
        const updateTrackRequests = changes.filter(c =>
            c.trackId != undefined &&
            c.changeType === ChangeType.UPDATE);
        ...
        const videosToRemove = tracks.filter(t => !t.copy && t.type === TrackType.VIDEO).map(t => t.id).join(",");
        if (videosToRemove) {
            mkOptions.push("--video-tracks", "!" + videosToRemove);
        }

        const audiosToRemove = tracks.filter(t => !t.copy && t.type === TrackType.AUDIO).map(t => t.id).join(",");
        if (audiosToRemove) {
            mkOptions.push("--audio-tracks", "!" + audiosToRemove);
        }

        const subtitlesToRemove = tracks.filter(t => !t.copy && t.type === TrackType.SUBTITLES).map(t => t.id).join(",");
