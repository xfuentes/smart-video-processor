...
import Processes from "../../util/processes.ts";
import Track from "../Track.ts";
import Numbers from "../../util/numbers.ts";
import {Attachment} from "../Change.ts";
    ...
    public async completeFileInformation(path: string, tracks: Track[], container: Container): Promise<void> {
        ...
        probeInfo?.streams?.forEach((stream) => {
            ...
            if (track) {
                ...
                if (track.properties.videoDimensions === undefined && stream.width !== undefined && stream.height !== undefined) {
