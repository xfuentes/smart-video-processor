...
import EpisodeData from "../EpisodeData.ts";
...
export class TVDBClient {
    ...
    public async searchSeries(name: string, year: number | undefined = undefined): Promise<SearchResult[]> {
        ...
        for (const r of response.data.data) {
            ...
            const imdb = r.remote_ids?.filter(rObj => rObj.sourceName === "IMDB").map(rObj => rObj.id).pop();
            ...
            results.push(new SearchResult(Number.parseInt(r.tvdb_id), name, Number.parseInt(r.year), originalName, overview, r.image_url, language, country ? [country] : [],
                imdb));
        }
        return results;
    }

    public async retrieveSeriesDetails(tvdbId: number, order: EpisodeOrder, episodeNumber: number | undefined, absoluteEpisodeNumber: number | undefined, season: number | undefined): Promise<SeriesEpisode> {
        const tvdb = await this.getTVDBSession();
        ...
        const result = {
            episodeData: new EpisodeData(episodeData.number, episodeData.seasonNumber, episodeData.absoluteNumber, episodeData.name, episodeData.image, episodeData.overview),
            seriesData: new SearchResult(seriesData.id, name, Number.parseInt(seriesData.year), name,
                seriesData.overview, seriesData.image, language, country ? [country] : [])
        };
        let langCode = currentSettings.favoriteLanguages[0] ?? "en";
        if(langCode.indexOf("-") != -1) {
            langCode = langCode.substring(0, langCode.indexOf("-"));
        ...
        if(trCode != undefined) {
            ...
            try {
                const episodesTranslation = await tvdb.get<TVDBEpisodesTranslationResponse>(`/series/${tvdbId}/episodes/${order}/${trCode}`);
                ...
                for(const episode of episodesTranslation.data.data.episodes) {
                    if( ( (order === "official" || order === "dvd") && episode.seasonNumber === season && episode.number === episodeNumber)
