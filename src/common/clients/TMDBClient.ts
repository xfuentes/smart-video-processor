...
const TMDB_TOKEN = "eyJhbGciOiJIUzI1NiJ9.eyJhdWQiOiJkMmVmODJlOTcwNmE4NjVjN2IzYmJjMTlkMzczNWUxYSIsIm5iZiI6MTU3NjA1MDI1MC41NTAwMDAyLCJzdWIiOiI1ZGYwOWU0YWVkYTRiNzAwMTUwNDMyM2YiLCJzY29wZXMiOlsiYXBpX3JlYWQiXSwidmVyc2lvbiI6MX0.dtLAYJZYn_skkO7fbp_fMF61QbbAlihCAJAkVR8BOVo";
...
export class TMDBClient {
    ...
    public async searchMovieByImdb(imdb: string): Promise<SearchResult[]> {
        ...
        for (const r of response.data.movie_results) {
            results.push(new SearchResult(r.id, r.title, new Date(r.release_date).getFullYear(), r.original_title, r.overview,
                TMDB_IMAGE_URL + r.poster_path, Languages.getLanguageByCode(r.original_language)));
        }
        return results;
    }
    ...
    public async retrieveMovieDetails(tmdbId: number): Promise<MovieData> {
        ...
        return new MovieData(movieDetails.id,
