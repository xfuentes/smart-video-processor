...
import {TMDBClient} from "./clients/TMDBClient.ts";
...
export default class Movie {
    ...
    async search(by: SearchBy) {
        ...
        if (by === SearchBy.TMDB) {
            ...
            if (by === SearchBy.TITLE) {
                this.searchResults = await TMDBClient.getInstance().searchMovieByNameYear(this.title, this.year);
            } else if(this.imdb) {
            ...
            if (!movieMatched) {
                ...
                if (by === SearchBy.TITLE) {
                    this.video.message = "Unable to find an exact match on TMDB. Please check the information provided and try again.";
                } else {
    ...
    async load() {
        ...
        try {
            ...
            this.originalCountries = movieData.countries.map(c => Countries.getCountryByCode(c)).filter(c => c != undefined);
            ...
            if(this.duration != undefined && Math.round(this.duration / 1000) < Math.round((tracksDuration - (tracksDuration / 10)) / 1000)) {
            ...
            if(this.searchResults.length === 0) {
                this.searchResults.push(new SearchResult(this.tmdb, this.title, this.year,this.title, this.overview,
                    this.posterURL, this.originalLanguage,
                    this.originalCountries, this.imdb, this.rating));
            }
