...
import Processes, {ProcessesPriority, Progression} from "../../util/processes.ts";
...
export default abstract class Job<T> {
    ...
    updatePriority() {
        if (this.progression.process?.pid) {
            Processes.setPriority(this.progression.process.pid, ProcessesPriority[currentSettings.priority ?? defaultSettings.priority]);
        }
    }
