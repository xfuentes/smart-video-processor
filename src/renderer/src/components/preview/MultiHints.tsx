        ...
        <>
          ...
          <div className="hints-form">
            {languageHints.map((hint) => {
              ...
              const track = videos[0].tracks.find((t) => t.id === hint.trackId)
              return (
                <Field
                  key={key}
                  size="small"
                ...
                >
                  <LanguageSelector
                    ...
                    onChange={async (value) => {
                      if (value) {
                        await window.api.video.setMultiHint(
                          videos.map((v) => v.uuid),
                          hint,
                          value
                        )
            ...
            {subtitlesTypeHints.map((hint) => {
              ...
              return (
                <Field
                  key={key}
                  size="small"
                  label={`${track?.type ?? _('track.unknown', { defaultValue: 'Unknown' })} ${hint.trackId}`}
                  required
                  className={disabled ? 'disabled' : ''}
                >
                  <Select
