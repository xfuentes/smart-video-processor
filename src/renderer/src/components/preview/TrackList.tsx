...
import {bitrateRenderer, booleanRenderer, codecRenderer, durationRenderer, framesRenderer, sizeRenderer, trackPropertiesRenderer, trackTypeRenderer} from "./renderers.tsx";
import Video from "../../common/Video.ts";
import React from "react";
import xor from "lodash/xor";

const columns: TableColumnDefinition<Track>[] = [
    createTableColumn<Track>({
        columnId: "id",
...
export const TrackList = ({video}: Props) => {
    const handleSelectionChange: DataGridProps["onSelectionChange"] = (_e: React.KeyboardEvent | React.MouseEvent<Element, MouseEvent>, data: OnSelectionChangeData) => {
        const changedItems = xor(Array.from(selectedItems), Array.from(data.selectedItems as Set<number>));
        video.switchTrackSelection(changedItems);
    };
