...
import Video, {SearchBy, VideoType} from "../../src/common/Video";
import {expect, test} from "vitest";

test('TV-Show extracts season and episode number', () => {
    const video = new Video("c:\\out put\\One Piece - S12E003 - Le rêve sombrant dans le nouveau monde ! Le pirate du désespoir, Puzzle.mkv");
...
test('TV-Show extracts tvdb ID', async () => {
    const video = new Video("c:\\out put\\T.W.D.The.Ones.Who.Live.S01E01.MULTi.1080p.WEBRip.DDP5.1.HEVC-BATGirl{TVDB-427202}.mkv");
    expect(video.type).toBe(VideoType.TV_SHOW);
...
test('TV-Show extracts imdb ID', async () => {
    const video = new Video("c:\\out put\\T.W.D.The.Ones.Who.Live.S01E01.MULTi.1080p.WEBRip.DDP5.1.HEVC-BATGirl{TT9859436}.mkv");
    expect(video.type).toBe(VideoType.TV_SHOW);
    expect(video.tvShow.season).toBe(1);
    expect(video.tvShow.episode).toBe(1);
...
test('Movie extracts tmdb ID', async () => {
    const video = new Video("C'Est.Pas.Parce.Qu'On.A.Rien.À.Dire.Qu'Il.Faut.Fermer.Sa.Gueule.(1975).FRENCH.HDLight.1080p.AAC.x264-Notag{tmdb-58652}.mkv");
...
test('Movie Search by TMDB', async () => {
    ...
    expect(video.movie.searchResults.length).toBe(1);
    expect(video.movie.searchResults[0].id).toBe(81022);
    expect(video.movie.searchResults[0].title).toBe("Widows");
    expect(video.movie.searchResults[0].year).toBe(2011);
});
