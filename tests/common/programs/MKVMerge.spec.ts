...
const genSpawnSpyFileInfo = (jsonFileName: string) => vi.spyOn(Processes, "spawn").mockImplementation(
    (): ChildProcessWithoutNullStreams =>
        simulateFileInfoResponse(jsonFileName)
);

const genSpawnSpy = () => vi.spyOn(Processes, "spawn").mockImplementation(
    (): ChildProcessWithoutNullStreams => {
        return {
...
const genSpawnSpyProgress = () => vi.spyOn(Processes, "spawn").mockImplementation(
    (command: string, args: readonly string[], options: SpawnOptionsWithStdioTuple<StdioPipe, StdioPipe, StdioPipe>): ChildProcessWithoutNullStreams => {
        let dataListener = undefined;
        let closeListener = undefined;
...
test('MKVMerge Electric State File Info', async () => {
    ...
    expect(tracks[0]).toStrictEqual(new Track(0, undefined, TrackType.VIDEO, "HEVC/H.265/MPEG-H", "und", {
    ...
    }, true, false, 1156400719, 7709.917));
    expect(tracks[1]).toStrictEqual(new Track(1, undefined, TrackType.AUDIO, "E-AC-3", "fr", {
        "audioChannels": 6,
    ...
    }, true, false, 740152320, 7709.92));
    expect(tracks[7]).toStrictEqual(new Track(7, "SDH", TrackType.SUBTITLES, "SubRip/SRT", "en", {
        "audioChannels": undefined,
        "audioSamplingFrequency": undefined,
        "bitRate": 74,
        "frames": 1947,
...
test('MKVMerge Fermer Gueule File Info', async () => {
    ...
    expect(tracks[0]).toStrictEqual(new Track(0, undefined, TrackType.VIDEO, "AVC/H.264/MPEG-4p10", undefined, {
        "audioChannels": undefined,
        "audioSamplingFrequency": undefined,
        "bitRate": undefined,
        "frames": undefined,
        "textSubtitles": undefined,
        "videoDimensions": "1920x1080",
    }, true, false, undefined, 5375.563));
    expect(tracks[1]).toStrictEqual(new Track(1, undefined, TrackType.AUDIO, "AAC", "fr", {
        "audioChannels": 2,
        "audioSamplingFrequency": 48000,
        "bitRate": undefined,
        "frames": undefined,
...
test('MKVMerge Le Fils Du Cordonnier File Info', async () => {
    ...
    expect(container.attachments?.length).toBe(0);
    expect(tracks.length).toBe(2);
    expect(tracks[0]).toStrictEqual(new Track(0, undefined, TrackType.VIDEO, "MPEG-4p2", undefined, {
        "audioChannels": undefined,
        "audioSamplingFrequency": undefined,
        "bitRate": undefined,
        "frames": undefined,
    ...
    expect(tracks[1]).toStrictEqual(new Track(1, undefined, TrackType.AUDIO, "AC-3", undefined, {
        "audioChannels": 2,
        "audioSamplingFrequency": 48000,
        "bitRate": undefined,
        "frames": undefined,
...
test('MKVMerge Progression data', async () => {
    ...
    await MKVMerge.getInstance().processFile(
        pathBasename(fullPath),
        fullPath,
        "C:\\Download\\ReworkedTest", [], [], progression => {
            progresses.push(progression.progress);
        });

    expect(spawnSpy).toHaveBeenCalled();
    expect(progresses).toStrictEqual([0, 0.25, 0.50, 0.75, 1,]);
});
...
test('MKVMerge Filename changed', async () => {
    ...
    const fullPath = "C:\\Download\\something.mkv";
    await MKVMerge.getInstance().processFile(pathBasename(fullPath), fullPath, "C:\\Download\\ReworkedTest", changes);

    expect(spy).toHaveBeenCalled();
    const spawnArgs = spy.mock.lastCall[1];
    const outputIdx = spawnArgs.indexOf("--output");
    expect(outputIdx, "--output not found").toBeGreaterThanOrEqual(0);
    expect(spawnArgs[outputIdx + 1]).toBe("C:\\Download\\ReworkedTest\\" + changesMap["Container"]["Update_Filename"].newValue);
});
...
test("Title changed", async () => {
    ...
    const changes = [
        ...
        },
        {
            ...
            "changeType": "Attach",
            "property": "Poster Image",
            "newValue": "C:\\Users\\xfuentes\\Documents\\Projets\\smart-video-processor\\tmp-svp-TXMU1k\\TMDB-1197306\\cover.jpg"
        },
        {
...
test('MKVMerge Provokes error', async () => {
    ...
    const fullPath = "C:\\Download\\something.mkv";
    await expect(MKVMerge.getInstance().processFile(pathBasename(fullPath), fullPath, "C:\\Download\\ReworkedTest", []))
...
test("Tags And Attachments updated", async () => {
    ...
    expect(spawnArgs[attachmentDescriptionIdx + 1]).toBe("TMDB Poster https://image.tmdb.org/t/p/w1280/bH7kGBVR5d2T984reMaxsAnj6oJ.jpg");
    ...
    expect(spawnArgs[attachFileIdx + 1]).toBe("C:\\Users\\xfuentes\\Documents\\Projets\\smart-video-processor\\tmp-svp-BVytyg\\TMDB-124418\\cover.jpg");
});
...
test("Track Properties updated", async () => {
    ...
    expect(spy).toHaveBeenCalled();
    const spawnArgs = spy.mock.lastCall[1];
    const defaultIndexes = spawnArgs.map((arg, index) => (arg === "--default-track") ? index : undefined).filter(arg => arg !== undefined);
    ...
    const forcedIndexes = spawnArgs.map((arg, index) => (arg === "--forced-track") ? index : undefined).filter(arg => arg !== undefined);
    expect(forcedIndexes.length).toBe(1);
    expect(spawnArgs[forcedIndexes[0] + 1]).toBe("2:yes");

    const namedIndexes = spawnArgs.map((arg, index) => (arg === "--track-name") ? index : undefined).filter(arg => arg !== undefined);
    ...
    const languageIndexes = spawnArgs.map((arg, index) => (arg === "--language") ? index : undefined).filter(arg => arg !== undefined);
...
test("Track Properties updated 2", async () => {
    ...
    const fullPath = "C:\\Download\\savate.mkv";
    await MKVMerge.getInstance().processFile(pathBasename(fullPath), fullPath, "c:\\ReworkedTest\\", changes);
    expect(spy).toHaveBeenCalled();
    const spawnArgs = spy.mock.lastCall[1];
    const defaultIndexes = spawnArgs.map((arg, index) => (arg === "--default-track") ? index : undefined).filter(arg => arg !== undefined);
    expect(defaultIndexes.length).toBe(2);
...
test("Track Removed", async () => {
    ...
    const fullPath = "C:\\Download\\savate.mkv";
    await MKVMerge.getInstance().processFile(pathBasename(fullPath), fullPath, "c:\\ReworkedTest\\", [], tracks);
    expect(spy).toHaveBeenCalled();
    const spawnArgs = spy.mock.lastCall[1];
