...
import {TVDBClient} from "../../../src/common/clients/TVDBClient";
...
test('Retrieve Series Captain Marleau', async () => {
    ...
    expect(episode.episodeData.posterURL).toBe("https://artworks.thetvdb.com/banners/v4/episode/8141218/screencap/606fff72bc7a6.jpg");
    expect(episode.episodeData.absoluteNumber).toBe(23);
    expect(episode.episodeData.overview).toContain("Une vieille connaissance de Marleau est accusée de meurtre. La victime ? Le patron d’une pêcherie de Royan. La capitaine doit composer avec Alex Weller, un flic renommé que le procureur a co-saisi, et qui semble enquêter à charge. Entre trafic de cigarettes, précarité et revanche sociale, les mobiles fleurissent autour des habitants d'un quartier menacé de destruction. Mais les soupçons de Marleau se portent bientôt sur Weller : pourquoi a-t-il tenu à être chargé de l’enquête ? Y aurait-il un lien avec sa fille, autrefois retrouvée morte alors qu’elle était en vacances dans la région ?");
    expect(episode.seriesData.posterURL).toBe("https://artworks.thetvdb.com/banners/v4/series/316274/posters/648dd5cec300d.jpg");
    expect(episode.seriesData.title).toBe("Capitaine Marleau");
    expect(episode.seriesData.id).toBe(316274);
    expect(episode.seriesData.overview).toContain("Elément atypique de la gendarmerie nationale, la capitaine Marleau détonne par son franc-parler, sa gouaille et son accent ch'ti. Sarcastique, déconcertante, coiffée d'une improbable chapka, elle promène sa silhouette filiforme et son flegme sur les lieux de crime divers.");
    expect(episode.seriesData.imdb).toBe(undefined);
    expect(episode.seriesData.countries.length).toBe(1);
...
test('Rate limiter', async () => {
    ...
    const prom1 = TVDBClient.getInstance().retrieveSeriesDetails(79168, "official", 10, undefined, 2).then((_result) => {
        firstExecutedAt = Date.now();
    });
    let secondExecutedAt: number;
    const prom2 = TVDBClient.getInstance().retrieveSeriesDetails(79168, "official", 5, undefined, 2).then((_result) => {
        secondExecutedAt = Date.now();
    });
    let thirdExecutedAt: number;
    const prom3 = TVDBClient.getInstance().retrieveSeriesDetails(79168, "official", 4, undefined, 2).then((_result) => {
...
test('Search Street Hawk', async () => {
    ...
    expect(series[0].overview).toContain("Jesse Mach, motard au sein de la police, est blessé au cours d'une opération. " +
...
test('Retrieve Walking Dead Dead City Details in French', async () => {
    ...
    expect(episode.episodeData.posterURL).toBe("https://artworks.thetvdb.com/banners/v4/episode/10803215/screencap/684e9572af0f5.jpg");
    expect(episode.episodeData.absoluteNumber).toBe(13);
    expect(episode.episodeData.overview).toContain("Negan et Maggie s'embarquent dans des missions éprouvantes et font face à des épreuves inattendues, autant physiques qu'émotionnelles.");
    expect(episode.seriesData.posterURL).toBe("https://artworks.thetvdb.com/banners/v4/series/417549/posters/64d0c1764319a.jpg");
...
test('Retrieve Vic le Viking Details in French DVD order', async () => {
    ...
    expect(episode.episodeData.overview).toContain("Nos vikings sont tombés dans un piège : un groupe de pirates leur a fait croire à l'existence d'un trésor fabuleux. " +
    ...
    expect(episode.seriesData.overview).toBe("Vic, le fils du chef Viking Halvar accompagne régulièrement son père et son équipage" +
...
test('Shameless US Cleanup', async () => {
    ...
    expect(episode.episodeData.overview).toContain("Depuis que leur mère, Monica, a quitté la maison, les enfants Gallagher se débrouillent " +
    ...
    expect(episode.seriesData.overview).toBe("Frank Gallagher est le père d'une famille dysfonctionnelle vivant dans un quartier défavorisé. " +
