...
test('Search by IMDB ID', async () => {
    ...
    expect(movies[0].overview).toBe("Set in the future, the story follows a young soldier named Johnny Rico and his exploits " +
...
test('Search by name', async () => {
    ...
    expect(movies[0].overview).toBe("An American soldier who had been killed during the Vietnam War is revived 25 years later " +
...
test('Search by name and year', async () => {
    ...
    expect(movies[0].overview).toBe("After kidnapping a father and his two kids, " +
...
test('Retrieve movie details', async () => {
    ...
    expect(movie.overview).toBe("After a wizard's spell goes awry, 12th-century Gallic knight Godefroy de Papincourt, " +
...
test('Retrieve movie details issue with nobody has to know', async () => {
    ...
    expect(movie.overview).toBe("After a stroke and no memories from his past, Phil encounters Millie who inhabits a desert part of Isle of Lewis. She will shortly entrust him with his deepest secret : they were in love.");
    expect(movie.posterUrl).toMatch(posterUrlRE);
    expect(movie.language).toBe("fr");
    expect(movie.countries).toContain("FR");
...
test('Rate limiter', async () => {
    ...
    const prom1 = TMDBClient.getInstance().searchMovieByImdb("tt0120201").then((_result) => {
        firstExecutedAt = Date.now();
    });
    let secondExecutedAt: number;
    const prom2 = TMDBClient.getInstance().searchMovieByNameYear("universal soldier").then((_result) => {
        secondExecutedAt = Date.now();
    });
    let thirdExecutedAt: number;
    const prom3 = TMDBClient.getInstance().searchMovieByNameYear("une nuit en enfer", 1996).then((_result) => {
...
test('Search pacifist cowboy', async () => {
    ...
    expect(movies[0].overview).toBe("On l’appelait Trinita, mais son nom était Mario Girotti. Le western-spaghetti est né avec le " +
...
test('Retrieve movie details french', async () => {
    ...
    expect(movie.year).toBe(1993);
    expect(movie.overview).toBe("Parce qu'il lui a sauvé la vie lors d'une échauffourée avec les Anglais, " +
...
test('Retrieve movie details no french translations', async () => {
    ...
    expect(movie.overview).toBe("A destitute, crippled child and his mother are visited late one night " +
...
test('Retrieve movie details eternam 2', async () => {
    ...
    expect(movie.year).toBe(2022);
    expect(movie.overview).toBe("Paradis, Purgatoire, Enfer, Fin des temps, Apocalypse…  " +
...
test('Retrieve movie details passion of the Christ', async () => {
    ...
    expect(movie.overview).toBe("Les douze dernières heures de la vie du Christ. Rendu au Mont des Oliviers, " +
...
test('Retrieve movie details le bon la brute... with duration', async () => {
    ...
    expect(movie.overview).toBe("Pendant la guerre de Sécession, " +
...
test('Retrieve movie details ip man 3', async () => {
    ...
    expect(movie.overview).toBe("Lorsque qu'une bande de gangsters dirigée par un promoteur " +
...
test('check TMDB language id mappable to Language IETF', async () => {
    ...
    for(const tmdbLanguage of tmdbLanguages) {
        ...
        expect(Languages.getLanguageByCode(tmdbLanguage.iso_639_1), "Language not found [" + tmdbLanguage.iso_639_1 + "]").toBeDefined();
        pos++;
    }
});
