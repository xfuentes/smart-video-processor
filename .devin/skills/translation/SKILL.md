...
2. Run the script from the project root:
   ```bash
   node .devin/skills/translation/add-translation-key.js <dot.separated.key> ["English source text"] [--update]
   ```
   For new keys with the source already in the `_()` call, you can pass just the key:
   ```bash
   node .devin/skills/translation/add-translation-key.js <dot.separated.key> --update
   ```
3. The script writes the English source value to `locales/en.json` and a Google Translate generated value to every other `locales/<lang>.json`. Use `--update` to overwrite existing keys.
